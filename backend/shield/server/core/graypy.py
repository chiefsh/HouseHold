...
class BaseGELFHandler(object):
    def __init__(self, host, port=12201, chunk_size=WAN_CHUNK,
            ...
            localname=None, facility=None, level_names=False, compress=True, access_log=False):
    ...
    def makePickle(self, record):
        message_dict = make_message_dict(
            ...
            self.localname, self.level_names, self.facility, self.access_log)
...
class GELFHandler(BaseGELFHandler, DatagramHandler):
    ...
    def __init__(self, host, port=12201, chunk_size=WAN_CHUNK,
        ...
        BaseGELFHandler.__init__(self, host, port, chunk_size,
                                 ...
                                 localname, facility, level_names, compress, access_log)
...
def make_message_dict(record, debugging_fields, extra_fields, fqdn, localname,
                      level_names, facility=None, access_log=False):
    if fqdn:
        host = socket.getfqdn()
    elif localname:
        host = localname
    ...
    timestamp = hasattr(record, 'timestamp') and record.timestamp or record.created
    record.asctime = time.strftime('%y%m%d %H:%M:%S', time.localtime(timestamp))
    record.message = record.getMessage()
    fields = {'version': "1.0",
        'host': host,
        'app': options.PUSH_APP,
        'short_message': '[%(levelname)1.1s %(asctime)s %(module)s:%(lineno)d] %(message)s' % record.__dict__ if access_log else record.getMessage(),
...
graylog.propagate = False
graylog.setLevel(logging.INFO)
graylog.addHandler(GELFHandler(options.GELF_HOST, options.GELF_PORT, debugging_fields=False))
graylog_channel = GELFHandler(options.GELF_HOST, options.GELF_PORT, debugging_fields=False, access_log=True)
...
if __name__ == "__main__":
    ...
    handler = GELFHandler('localhost', 12202)
    my_logger.addHandler(handler)

    my_logger.debug('Hello Graylog2.', extra={'a': 'b', 'timestamp': 1284101485})
